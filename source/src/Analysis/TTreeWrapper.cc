...
//				delete itTreeMap->second.first;     // delete the TTree
	...
	TTreeWrapper::BranchHandler::~BranchHandler() {
		...
		if( m_branchFloat  != NULL ) delete m_branchFloat;
		if( m_branchDouble != NULL ) delete m_branchDouble;
		if( m_branchInt    != NULL ) delete m_branchInt;
		if( m_branchBool   != NULL ) delete m_branchBool;
		if( m_branchVectorFloat  != NULL ) delete m_branchVectorFloat;
		if( m_branchVectorDouble != NULL ) delete m_branchVectorDouble;
		if( m_branchVectorInt    != NULL ) delete m_branchVectorInt;
		if( m_branchVectorBool   != NULL ) delete m_branchVectorBool;
	}
	...
	bool TTreeWrapper::BranchHandler::Set(bool value) {

		if (m_branchBool && m_branchType != BRANCH_BOOL)
			return false;

		if (m_branchBool == NULL) {

			m_branchBool = new Branch<bool>(m_tree, m_branchName); ///< create a branch of type bool
			m_branchType = BRANCH_BOOL;
		}
		m_branchBool->Set(value);
		return true;
	}
	...
	bool TTreeWrapper::BranchHandler::Set(VectorBool *ptr) {

		if(m_branchVectorBool && m_branchType != BRANCH_VECTOR_BOOL)
			return false;

		if(m_branchVectorBool == NULL) {

			m_branchVectorBool = new Branch<VectorBool*>(m_tree, m_branchName); ///< create a branch of type VectorInt
			m_branchType = BRANCH_VECTOR_BOOL;
		}
		m_branchVectorBool->Set(ptr);
		return true;
	}
	...
	template TTreeWrapper::BranchMap::iterator TTreeWrapper::AddBranch<bool>(const std::string &, const std::string &);
	...
	template TTreeWrapper::BranchMap::iterator TTreeWrapper::AddBranch<VectorBool*>(const std::string &, const std::string &);
	...
	template bool TTreeWrapper::Set<bool>(const std::string &, const std::string &, bool);
	...
	template bool TTreeWrapper::Set<VectorBool*>(const std::string &, const std::string &, VectorBool*);
